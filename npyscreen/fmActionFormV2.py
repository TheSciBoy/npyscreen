...
class ActionFormV2(fmForm.FormBaseNew):
    ...
    class OKButton(wgbutton.MiniButtonPress):
        def whenPressed(self):
            return self.parent._on_ok()
    
    class CancelButton(wgbutton.MiniButtonPress):
        def whenPressed(self):
            return self.parent._on_cancel()
    
    OK_BUTTON_TYPE = OKButton
    CANCEL_BUTTON_TYPE = CancelButton
    ...
    def create_control_buttons(self):
        self._add_button('ok_button', self.__class__.OK_BUTTON_TYPE, self.__class__.OK_BUTTON_TEXT,
                         0 - self.__class__.OK_BUTTON_BR_OFFSET[0],
                         0 - self.__class__.OK_BUTTON_BR_OFFSET[1] - len(self.__class__.OK_BUTTON_TEXT), None)
                        
        self._add_button('cancel_button', self.__class__.CANCEL_BUTTON_TYPE, self.__class__.CANCEL_BUTTON_TEXT,
                         0 - self.__class__.CANCEL_BUTTON_BR_OFFSET[0],
                         0 - self.__class__.CANCEL_BUTTON_BR_OFFSET[1] - len(self.__class__.CANCEL_BUTTON_TEXT), None)
    ...
    def _add_button(self, button_name, button_type, button_text, button_rely, button_relx, button_function):
        ...
        this_button = self.add_widget(button_type, name=button_text, rely=button_rely, relx=button_relx,
                                      when_pressed_function=button_function, use_max_space=True)
